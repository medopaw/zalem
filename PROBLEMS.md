...
- 这违反了单一职责原则，使类难以测试和维护

### 2.2 依赖注入不完善

- 许多组件直接依赖具体实现而非接口：
  - 直接导入和使用 `supabase` 实例，而不是通过依赖注入
  - 虽然有一些接口定义（如 `IMessageRepository`），但使用不一致

### 2.3 缺乏清晰的分层架构

- 业务逻辑、数据访问和UI层之间的边界模糊
- 缺少明确的服务层、仓储层和表示层分离
